...
class HorarioCollector extends Collector
{
  function showTarea()
  {
    $rows = self::$db->getRows("SELECT cod_tarea,fecha,descripcion_mat,descripcion,descripcion_ta,fecha_entrega,hora,hora_ini, hora_fin FROM tarea 
                                
                                JOIN horario 
                                      ON horario.cod_horario=tarea.matxcur_cod
                                JOIN curso 
                                  ON curso.cod_curso=tarea.matxcur_cod
                                  
                                JOIN materia
                                  ON materia.cod_materia=tarea.matxcur_cod");
    $arrayTarea = array();
    foreach ($rows as $c) {
      $aux = new Tarea($c{'hora'},$c{'hora_ini'},$c{'hora_fin'},$c{'cod_tarea'},$c{'fecha'}, $c{'descripcion_mat'}, $c{'descripcion'}, $c{'descripcion_ta'}, $c{'fecha_entrega'});
  ...
  function showTareaCurso()
  {
    ...
    $rows = self::$db->getRows("SELECT cod_tarea,fecha,descripcion_mat,descripcion,descripcion_ta,fecha_entrega,hora,
                                       h_inicio, h_fin  FROM tarea   
                                JOIN curso 
